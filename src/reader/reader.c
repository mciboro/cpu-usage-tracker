...
unsigned int read_proc_stat(ring_buffer_t *const rbuf, char const *const proc_stat_dir) {
    if (!rbuf) {
        put_log("There is no ring buffer!");
        return VOID_ARG;
    }

    if (!proc_stat_dir) {
        put_log("There is no proc stat file direction!");
        return INV_ARG;
    }

    FILE *fptr = fopen(proc_stat_dir, "r");
    if (!fptr) {
        put_log("Error while opening proc stat file!");
        return FILE_ERROR;
    }
...
unsigned int read_cores_num(unsigned int *const result, char const *const proc_stat_dir) {
    ...
    if (!result) {
        put_log("There is no object to put in result!");
        return VOID_ARG;
    }
